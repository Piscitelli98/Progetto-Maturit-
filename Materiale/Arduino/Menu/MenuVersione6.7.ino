...
String userDaId="";
 String l_line = "";
  ...
  myFileLog = SD.open("log.txt");
 // if(verificaUtente(username)){
       if (myFileLog){
        //open the file here
        while (myFileLog.available() != 0){  
          Serial.println("dentro lettura file di log");
          //A inconsistent line length may lead to heap memory fragmentation        
          l_line = myFileLog.readStringUntil('\n');  
          //int trovPerc = l_line.indexOf('%'); 
          int trovPerc = l_line.length(); 
          int PuntoVirgola = l_line.indexOf(';');
          String id=l_line.substring(0, PuntoVirgola);     
         ...
         trovPerc = l_line.length();
          PuntoVirgola = l_line.indexOf(';');
          String ora_locale=l_line.substring(0, PuntoVirgola);

           l_line=l_line.substring(PuntoVirgola+1, trovPerc+1);
         trovPerc = l_line.length();
          PuntoVirgola = l_line.indexOf(';');
          String minuti_locale=l_line.substring(0, PuntoVirgola);

          String oraInv=ora_locale+":"+minuti_locale;
         
                //no blank lines are anticipated    
           Serial.println("data | data locale: "+ data +" | "+data_locale);
                  if(data==data_locale){
                    userDaId=findUserById(id);
                    //String userDaId="prv";
                    Serial.println("id e user corrispondente: "+ id+ " | "+ userDaId);
                    String tmp=userDaId+";"+data+";"+oraInv+";%";
                    Serial.println("tmp: "+tmp);
                    Serial.println("Messaggio compleno fino ora prima"+messaggioCompleto);
                    messaggioCompleto+=tmp;
  ...
  //sprintf(buffer, "%d", messaggioCompleto);
      String msInvio="DL_response;"+messaggioCompleto;
  //Serial3.write(buffer, strlen(buffer));
       Serial.println("invio file");
