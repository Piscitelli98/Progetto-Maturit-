...
int getFingerprintIDez() {
  ...
  //messaggio rimuovere impronta
  lcd.setCursor(0, 1); 
   lcd.print("Rimuovere Impronta");
   lcd.setCursor(4, 1);
...
switch (stato){
  case 0:
  {
     ...
     riceviSegnaleBTH();//metodo per attendere una connessione bluetooth
     key = keypad.getKey();
          if (key != NO_KEY){
                  if(key=='#'){
